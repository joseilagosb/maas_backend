...
describe IntervalManager do
  ...
  context 'with valid parameters' do
    ...
    context 'base availability' do
      let(:intervals) { IntervalManager.build_intervals(availabilities['base']) }
    ...
    context 'with two intervals in a day' do
      let(:intervals_with_two_intervals_in_a_day) do
        IntervalManager.build_intervals(availabilities['with_two_intervals_in_a_day'])
      end
    ...
    context 'with unavailable user in a day' do
      let(:intervals_with_unavailable_user) do
        IntervalManager.build_intervals(availabilities['with_unavailable_user'])
      end
    ...
    context 'empty availability' do
      let(:intervals_with_empty_availability) do
        IntervalManager.build_intervals(availabilities['empty'])
      end
  ...
  context 'with invalid parameters' do
    it 'raises an ArgumentError' do
      expect { IntervalManager.build_intervals(nil) }.to raise_error(ArgumentError)
    end
  end
end
